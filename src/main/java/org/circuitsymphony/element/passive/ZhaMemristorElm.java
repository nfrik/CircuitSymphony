...
public class ZhaMemristorElm extends CircuitElm {
    private double r_on;
    ...
    private double a; //a Cao & Sillin and alpha Chang
    private double b; //b Cao and betta Chang
    private double p; //g Biolek and Cao and gamma Chang
...
//    private double e; //delta Chang
    ...
    private double eps;
    private double tau;
//    private int memtype = MEM_BIOLEK;
...
//    protected static final int MEM_SILLIN = 6;
    ...
    public ZhaMemristorElm(CircuitEngine engine, int xx, int yy) {
        super(engine, xx, yy);
        r_on = 100;
        r_off = 160 * r_on;
        dopeWidth = 0;
...
//        memtype = MEM_SILLIN;
    ...
    public ZhaMemristorElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f,
                           StringTokenizer st) {
        this(engine, xa, ya, xb, yb, f, 0, st);
    }

    public ZhaMemristorElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f, int f2,
                           StringTokenizer st) {
...
//                totalWidth + " " + mobility + " " + g + " " + l + " " + tau;
        return super.dump(newFormat) + " " + r_on + " " + r_off + " " + dopeWidth + " " +
                totalWidth + " " + mobility + " " + a + " " + b + " " + p + " " + l + " " + tau;
    }
...
//        dopeWidth += mobility * r_on / totalWidth * current * windowBiolek() * engine.timeStep - dopeWidth / tau * engine.timeStep;
        dopeWidth += mobility * r_on / totalWidth * current * windowZha() * engine.timeStep - dopeWidth / tau * engine.timeStep;
        dopeWidth += (dopeWidth-totalWidth) / tau * engine.timeStep;
        if (dopeWidth <= eps)
            dopeWidth = eps;
...
//
//
    private double windowZha(){
        if(-current >= 0)
            return l*(1 - Math.pow(a * Math.pow((dopeWidth/totalWidth - 1),2*b) +(1-a), p));
        return l*(1 - Math.pow(a * Math.pow((dopeWidth/totalWidth),2*b) +(1-a), p));
    }
...
//        }
        if (n == 0)
            return new EditInfo("Ron (ohms)", r_on, 1, 1e5);
        if (n == 1)
            return new EditInfo("Roff (ohms)", r_off, 1, 1e5);
        if (n == 2)
            return new EditInfo("Width of Doped Region (nm)", dopeWidth * 1e9, 1e9, 1e12);
        if (n == 3)
            return new EditInfo("Total Width (nm)", totalWidth * 1e9, 1e9, 1e12);
        if (n == 4)
            return new EditInfo("Mobility (um^2/(s*V))", mobility * 1e12, 1e4, 1e14);
        if (n == 5)
            return new EditInfo("a", a , 0, 1e9);
        if (n == 6)
            return new EditInfo("b", b, 0, 5e2);
        if (n == 7)
            return new EditInfo("p", p, 0, 5e2);
//        if (n == 7)
//            return new EditInfo("Window scale", l , 0, 0);
        if (n == 8)
            return new EditInfo("Window max value", l, 0, 5e1);
//        if (n == 11)
//            return new EditInfo("e (eta Cao) (a.u.)", e, 0, 0);
        if (n == 9)
            return new EditInfo("tau (s)", tau, -1e1, 1e3);

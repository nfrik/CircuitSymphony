...
public class TransformerElm extends CircuitElm {
    ...
    public TransformerElm(CircuitEngine engine, int xx, int yy) {
        super(engine, xx, yy);
        inductance = 4;
        ratio = 1;
        width = 32;
    ...
    public TransformerElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f,
                          StringTokenizer st) {
        this(engine, xa, ya, xb, yb, f, 0, st);
    }

    public TransformerElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f, int f2,
                          StringTokenizer st) {
        ...
        couplingCoef = .999;
        try {
            couplingCoef = new Double(st.nextToken());
        } catch (Exception e) {
        }
        noDiagonal = true;
    }
    ...
    public void draw(DrawContext ctx) {
        ...
        for (i = 0; i != 2; i++) {
            setPowerColor(ctx, current[i] * (volts[i] - volts[i + 2]));
            drawCoil(ctx, dsign * (i == 1 ? -6 : 6),
                    ptCoil[i], ptCoil[i + 2], volts[i], volts[i + 2]);
        }
        ctx.setColor(needsHighlight(ctx) ? ctx.selectColor : ctx.lightGrayColor);
        for (i = 0; i != 2; i++) {
            ctx.drawThickLine(ptCore[i], ptCore[i + 2]);
            curcount[i] = updateDotCount(ctx, current[i], curcount[i]);
        }
        for (i = 0; i != 2; i++) {
            drawDots(ctx, ptEnds[i], ptCoil[i], curcount[i]);
            drawDots(ctx, ptCoil[i], ptCoil[i + 2], curcount[i]);
            drawDots(ctx, ptEnds[i + 2], ptCoil[i + 2], -curcount[i]);
        }
    ...
    public void stamp() {
        ...
        double ts = isTrapezoidal() ? engine.timeStep / 2 : engine.timeStep;
        a1 = l2 * deti * ts; // we multiply dt/2 into a1..a4 here
        a2 = -m * deti * ts;
        a3 = -m * deti * ts;
        a4 = l1 * deti * ts;
        engine.stampConductance(nodes[0], nodes[2], a1);
        engine.stampVCCurrentSource(nodes[0], nodes[2], nodes[1], nodes[3], a2);
        engine.stampVCCurrentSource(nodes[1], nodes[3], nodes[0], nodes[2], a3);
        engine.stampConductance(nodes[1], nodes[3], a4);
        engine.stampRightSide(nodes[0]);
        engine.stampRightSide(nodes[1]);
        engine.stampRightSide(nodes[2]);
        engine.stampRightSide(nodes[3]);
    }
    ...
    @Override
    public void doStep() {
        engine.stampCurrentSource(nodes[0], nodes[2], curSourceValue1);
        engine.stampCurrentSource(nodes[1], nodes[3], curSourceValue2);
    }
    ...
    public void getInfo(DrawContext ctx, String arr[]) {
        ...
        arr[5] = "I1 = " + ctx.getCurrentText(current[0]);
        arr[6] = "I2 = " + ctx.getCurrentText(current[1]);
    }

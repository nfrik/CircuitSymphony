...
public class SinghMemristorElm extends CircuitElm {
    private double r_on;
...
//    protected static final int MEM_SILLIN = 6;
    ...
    public SinghMemristorElm(CircuitEngine engine, int xx, int yy) {
        super(engine, xx, yy);
        r_on = 100;
        r_off = 160 * r_on;
        dopeWidth = 0;
...
//        memtype = MEM_SILLIN;
    ...
    public SinghMemristorElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f,
                                   StringTokenizer st) {
        this(engine, xa, ya, xb, yb, f, 0, st);
    }

    public SinghMemristorElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f, int f2,
                                   StringTokenizer st) {
...
//        dopeWidth += mobility * r_on / totalWidth * current * windowBiolek() * engine.timeStep - dopeWidth / tau * engine.timeStep;
        dopeWidth += mobility * r_on / totalWidth * current * windowSingh() * engine.timeStep - dopeWidth / tau * engine.timeStep;
...
//    }
    ...
    private double windowSingh(){
        double x = dopeWidth/totalWidth;
//        double val = l*(1-Math.pow(Math.pow(dopeWidth/totalWidth-0.5,2)+0.75,g));
        double val = l*Math.pow(x*(1f/(1f+Math.exp(current+(x-1f)))-1f/(1f+Math.exp(current-(x-1f)))),g);
        return val;
    }
...
//        }
        ...
        if (n == 5)
            return new EditInfo("Window rate of dec.", g , 0, 1e9);
//        if (n == 7)
...
//            return new EditInfo("d (delta Cao) (a.u.)", d , 0, 0);
        if (n == 6)
            return new EditInfo("Window max value", l, 0, 0);
//        if (n == 11)

...
public class PhotoResistorElm extends CircuitElm {
    ...
    public PhotoResistorElm(CircuitEngine engine, int xx, int yy) {
        super(engine, xx, yy);
        maxresistance = 1e9;
        minresistance = 1e3;
        createSlider();
    ...
    public PhotoResistorElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f,
                            StringTokenizer st) {
        this(engine, xa, ya, xb, yb, f, 0, st);
    }

    public PhotoResistorElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f, int f2,
                            StringTokenizer st) {
    ...
    private void createSlider() {
        ...
        if (engine.isUISupported()) {
            engine.createUI(label = new Label("Light Level", Label.CENTER));

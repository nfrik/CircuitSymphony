package org.circuitsymphony.element.chips;

import org.circuitsymphony.element.ChipElm;
import org.circuitsymphony.engine.CircuitEngine;

import java.util.StringTokenizer;

public class JKFlipFlopElm extends ChipElm {
    public JKFlipFlopElm(CircuitEngine engine, int xx, int yy) {
        super(engine, xx, yy);
    }

    public JKFlipFlopElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f,
                         StringTokenizer st) {
        this(engine, xa, ya, xb, yb, f, 0, st);
    }

    public JKFlipFlopElm(CircuitEngine engine, int xa, int ya, int xb, int yb, int f, int f2,
                         StringTokenizer st) {
        super(engine, xa, ya, xb, yb, f, f2, st);
        pins[4].value = !pins[3].value;
    }

    @Override
    public String getChipName() {
        return "JK flip-flop";
    }

    @Override
    public void setupPins() {
        sizeX = 2;
        sizeY = 3;
        pins = new Pin[5];
        pins[0] = new Pin(0, SIDE_W, "J");
        pins[1] = new Pin(1, SIDE_W, "");
        pins[1].clock = true;
        pins[1].bubble = true;
        pins[2] = new Pin(2, SIDE_W, "K");
        pins[3] = new Pin(0, SIDE_E, "Q");
        pins[3].output = pins[3].state = true;
        pins[4] = new Pin(2, SIDE_E, "Q");
        pins[4].output = true;
        pins[4].lineOver = true;
    }

    @Override
    public int getPostCount() {
        return 5;
    }

    @Override
    public int getVoltageSourceCount() {
        return 2;
    }

    @Override
    public void execute() {
        if (!pins[1].value && lastClock) {
            boolean q = pins[3].value;
            if (pins[0].value) {
                q = !pins[2].value || !q;
            } else if (pins[2].value)
                q = false;
            pins[3].value = q;
            pins[4].value = !q;
        }
        lastClock = pins[1].value;
    }

    @Override
    public int getDumpType() {
        return 156;
    }
}
